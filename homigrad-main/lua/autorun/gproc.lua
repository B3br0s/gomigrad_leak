...
local function char(_offset)
	if _offset == nil then return source:sub(offset, offset) end
...
local function gprocError(err, _offset, charoffset)
	errordata = {
		error = err,
		offset = _offset or offset,
		charoffset = charoffset or 0
	}
...
local function parseStringInterpolation(type, s, contest)
	...
	while not EOF() do
		...
		if captureFormat and not captureArgs then
			if c == "," then
				captureArgs = true
			else
				-- "${a:a()} ${a:a''} ${a:a""} ${a:a{}} ${a:a[[]]}"
	...
	if captureFormat and _G["gprocfmt" .. format] == nil then
...
function parseString(interploatestring)
	...
	if c == "'" or c == "\"" then
		...
		while not EOF() do
			...
			if interploatestring and char() == "$" and char(1) == "{" then
				seek(2)
				contest = parseStringInterpolation(0, c, contest)
				continue
			end

			if c == char() and (bscount == 0 or bscount % 2 == 0) then
				next()
				break
			end
	...
	if char() == "[" and (char(1) == "[" or char(1) == "=") then
		local bscount = 0
		seek(1)
		local sign = match("=*") or ""
		seek(#sign + 1)
		...
		while not EOF() do
			...
			if match(pattern) then break end
			contest = contest .. char()
			next()
		end
...
local function parseFunction()
	...
	local function captureEnd()
		...
		if captureDefaultValue then
			if defaultValue:match("%s*!err$") then
				code = code .. "if " .. argName .. " == nil then error('bad argument #" .. argNum .. " " .. string.format("%q", argName):sub(2, -3) .. " (nil)') end "
			elseif defaultValue:match("%s*!ret") then
	...
	while not EOF() do
		...
		if c == "{" then
			sqrbracketsExcepted = sqrbracketsExcepted + 1
		end
		...
		if c == ")" then
			if bracketsExcepted == 0 and endExcepted == 0 then
				if argName ~= "" then
					captureEnd()
		...
		if c == "," and sqrbracketsExcepted == 0 and endExcepted == 0 and bracketsExcepted == 0 then
			captureEnd()
			next()
			continue
		end

		if captureArgName and c == "=" then
...
local function parseIf(type)
	...
	while not EOF() do
		if char() == "#" and (char(-1) == "\n" or offset == 1) then
			...
			if isValid then
				if derectiveName ~= "endif" and derectiveName ~= "elseif" then
					parseDerective(derectiveName)
				end
			end

			if derectiveName == "endif" then
				offset = sOffset

				return
			elseif derectiveName == "elseif" then
				if isValid then
					removeChars = true
				else
					removeChars = false
...
function parseDerective(derectiveName)
	if derectiveName == "define" then
		parseDefine()
	elseif derectiveName == "undef" then
		parseUnDefine()
	elseif derectiveName == "ifdef" then
		parseIf(IFDEF)
	elseif derectiveName == "ifndef" then
		parseIf(IFNDEF)
	elseif derectiveName == "if" then
		parseIf(IF)
	else
...
local function parseMacroArgs(name)
	...
	while not EOF() do
		...
		if c == "," and sqrBracketsExcepted == 0 and bracketsExcepted == 0 and endExcepted == 0 then
			args[#args + 1] = arg
			arg = ""
			next()
			continue
...
local function parseTier0()
	...
	while not EOF() do
		...
		if (char() == "/" and char(1) == "/") or (char() == "-" and char(1) == "-" and not char(-1):match("[%w%u]") and char(2) ~= "[") then
			while not EOF() do
				if char() == "\n" then break end
				newSource = newSource .. " "
				next()
		...
		if char() == "-" and char(1) == "-" and char(2) == "[" then
			seek(3)
			newSource = newSource .. "   "

			for i = 1, #(match(".*%]" .. (match("=*") or "") .. "%]") or "") do
				newSource = newSource .. (char() == "\n" and "\n" or " ")
				next()
			end
		end
...
local function parseTier1()
	...
	while not EOF() do
		...
		if (char() == "+" and char(1) == "+") or (char() == "-" and char(1) == "-") then
			local op = char()
	...
	if node == nil then
		if oNode and oNode.value and name ~= lName then
			source = source:sub(0, offset - clen) .. tostring(oNode.value) .. source:sub(offset)
			len = #source
			seek(-clen)
		end
...
local function onError(err)
	if err:sub(#err - 9) == "gprocerror" then
		offset = 0
		local line = 0
		local nLineOffset = 0

		while offset < errordata.offset do
			local _, _, _, linescount = parseString()
			line = line + linescount
...
local function parse(_source, _path)
	...
	if not errordata.error then
		xpcall(parseTier1, onError)
	end

	if not errordata.error then
		xpcall(parseTier2, onError)
	end

	if not errordata.error then
		xpcall(parseTier3, onError)
	end
...
local function filewatcher()
	...
	for k, filedata in pairs(files) do
		local time = fTime(k, "LUA")

		if filedata[1] ~= time then
			filedata[1] = time
			gproclib.include(k)
		end
	end
