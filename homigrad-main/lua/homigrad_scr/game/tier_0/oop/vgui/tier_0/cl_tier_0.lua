-- "lua\\homigrad_scr\\game\\tier_0\\oop\\vgui\\tier_0\\cl_tier_0.lua"
...
function PANEL:OnMouseWheeled(wheel)
	self:Event_Call("Mouse Wheel", -wheel)
end
...
if not vgui.hSetParent then
	vgui.hSetParent = Panel.SetParent
end
...
function Panel:SetParent(panel)
	...
	if self.Event_Call then
		result = self:Event_Call("Parent", panel, self)
	end

	if panel.Event_Call then
		result2 = panel:Event_Call("Parent", panel, self)
	end
	...
	if TypeID(result) == TYPE_PANEL then
		parent = result
	end

	if TypeID(result2) == TYPE_PANEL then
		panel = result2
	end
