-- "lua\\homigrad_scr\\game\\tier_1\\util_sh.lua"
...
if SERVER then
	util.AddNetworkString("CHEDON_DebugZonePoint")

local function GetRandomGroundPos()

end

timer.Create("CHEDON_DebugRandomPos", 1, 20, function()
end)
...
if CLIENT then
	...
	local debugZonePoints = {}
	local zoneRadius = 500
	local shrinkCenter = Vector(0, 0, 0)
	local zoneMaterial = Material("models/props_c17/oildrum001_explosive")
	
	hook.Add("PostDrawTranslucentRenderables", "CHEDON_DebugZoneVisual", function()
	end)
	
	hook.Add("Think", "CHEDON_ZoneDamage", function()	
	end)
...
if SERVER then
    ...
    hook.Add( "PlayerSay", "OnPlayerSayTTS", function( sender, text )
		if text[1] == "!" or text[1] == "/" or text[1] == "*" then return end
        if table.HasValue(ttsgroup, sender:GetUserGroup()) then
			net.Start( "PlayTTS" )
        	    net.WriteEntity( sender )
        	    net.WriteString( "google" )
        	    net.WriteString( "zahar" )
...
elseif CLIENT then
    ...
    net.Receive( "PlayTTS", function()
        ...
        tts_type = net.ReadString()
        tts_speaker = net.ReadString()
        tts_emotion = net.ReadString()
        text = UrlEncode( net.ReadString() )
        url = list.GetForEdit( "TTSUrls" )[ tts_type ]( tts_speaker, tts_emotion, text )
        is_3d_sound = GetConVar( "cl_tts_3d_sound" ):GetBool()
        sound_type = is_3d_sound and "3d" or "mono"
