-- "lua\\homigrad_scr\\game\\tier_1\\appearance\\appearance_cl.lua"
...
function EasyAppearance.SaveData(tAppearanceData)
	if not file.Exists("homigradcom", "DATA") then file.CreateDir("homigradcom") end

	file.Write("homigradcom/appearancedata.json", util.TableToJSON(tAppearanceData, true))
...
function EasyAppearance.GetData(bForceRandom)
	...
	if not file.Exists("homigradcom/appearancedata.json", "DATA") then return true end

	local tAppearanceData = util.JSONToTable(file.Read("homigradcom/appearancedata.json", "DATA"))
