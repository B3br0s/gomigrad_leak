...
function SolidMapVote.close()
	if IsValid(SolidMapVote.Menu) then
...
function SolidMapVote.GetMapConfigInfo(map)
	for _, mapData in pairs(SolidMapVote["Config"]["Specific Maps"]) do
		if map == mapData.filename then return mapData end
	end
...
hook.Add("PlayerBindPress", "SolidMapVote.StopMovement", function(ply, bind)
	if IsValid(SolidMapVote.Menu) and SolidMapVote.Menu:IsVisible() and bind ~= "solidmapvote_test" and (bind ~= "messagemode" and SolidMapVote["Config"]["Enable Chat"]) and (bind ~= "messagemode2" and SolidMapVote["Config"]["Enable Chat"]) and (bind ~= "+voicerecord" and SolidMapVote["Config"]["Enable Voice"]) then return true end
end)
...
concommand.Add("solidmapvote_nomination_menu", function()
	...
	if SolidMapVote.isNominating then
		if IsValid(SolidMapVote.Nominate) then
