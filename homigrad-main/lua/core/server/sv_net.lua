...
concommand.Add("solidmapvote_vote", function(ply, cmd, args)
	...
	if table.HasValue(SolidMapVote.maps, vote) or (vote == "extend" and SolidMapVote["Config"]["Enable Extend"]) or (vote == "random" and SolidMapVote["Config"]["Enable Random"]) then
...
concommand.Add("solidmapvote_nominate", function(ply, cmd, args)
	...
	if #SolidMapVote.nominations >= math.min(6, #SolidMapVote.mapPool) and not SolidMapVote.playerHasNominated(steamId64) then
	...
	-- needs to be in the map pool
	if table.HasValue(SolidMapVote.mapPool, nomination) and not table.HasValue(SolidMapVote.nominations, nomination) and SolidMapVote["Config"]["Allow Nominations"] then
		-- needs to not already be nominated
		-- nominations need to enabled
