...
function SolidMapVote.selectMaps()
	...
	while i < 6 do
		local map = SolidMapVote["Config"]["Fair Map Recycling"] and SolidMapVote.selectRandomMapFairly(SolidMapVote.mapPool, SolidMapVote.mapPlayCounts) or table.Random(SolidMapVote.mapPool)
...
function SolidMapVote.selectRandomMapFairly(pool, weights)
	...
	return nil -- Map not found? Return nil to retry
end