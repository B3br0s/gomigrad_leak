-- "gamemodes\\homigradcom\\gamemode\\game\\level_cl.lua"
...
local timef = 0
hook.Add("HUDPaint", "homigradcom-roundstate", function()
	if roundActive then
		
		if roundActiveName != "event" then
			if input.IsKeyDown(KEY_TAB) then
				timef = Lerp(0.1, timef, 0)
			else
				timef = Lerp(0.1, timef, 250)
			end

			local alphatimer = 255 - timef
			local remainingTime = math.max(0, (roundTimeStart + (5 * 60)) - CurTime())
			local minutes = math.floor(remainingTime / 60)
			local seconds = math.floor(remainingTime % 60)
			local timerText = string.format("%02d:%02d", minutes, seconds)
	
			draw.SimpleTextOutlined(timerText, "SolidMapVote.Title", ((ScrW() / 1.87)+5), ((ScrH() / 26)+5), Color(0, 0, 0, alphatimer), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 0.1, Color(0,0,0,alphatimer))
			draw.SimpleTextOutlined(timerText, "SolidMapVote.Title", (ScrW() / 1.87), (ScrH() / 26), Color(255, 255, 255, alphatimer), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 0.1, Color(0,0,0,alphatimer))
		end
...
hook.Add("PostDrawOpaqueRenderables", "homigradcom-PostDrawOpaqueRenderables", function()
