-- "gamemodes\\homigradcom\\entities\\entities\\ent_aidbox_big.lua"
...
if SERVER then
	...
	function ENT:PhysicsCollide(data, physobj)
		if data.Speed > 2000 and data.DeltaTime > .2 then
			...
			if Tr.Hit then util.Decal("Rollermine.Crater", Tr.HitPos + Tr.HitNormal, Tr.HitPos - Tr.HitNormal) end
	...
	local function SpawnItem(itemClass, pos, owner, resourceAmt)
		...
		if ItemNameParts and ItemNameParts[1] == "FUNC" then
			if ItemNameParts[2] and JMod.LuaConfig.BuildFuncs[ItemNameParts[2]] then JMod.LuaConfig.BuildFuncs[ItemNameParts[2]](owner, pos + Vector(0, 0, 5), Angle(0, 0, 0)) end
		else
			...
			if IsValid(Yay) then
				...
				timer.Simple(.025, function() if IsValid(Yay) then Yay:GetPhysicsObject():SetVelocity(Vector(0, 0, 0)) end end)
				timer.Simple(.05, function() if IsValid(Yay) then Yay:GetPhysicsObject():SetVelocity(Vector(0, 0, 0)) end end)
				timer.Simple(.1, function() if IsValid(Yay) then Yay:GetPhysicsObject():SetVelocity(Vector(0, 0, 0)) end end)
			end
		end
	end
	...
	local function SpawnContents(contents, pos, owner)
		...
		if typ == "string" then
			return SpawnItem(contents, pos, owner)
	...
	function ENT:Use(activator, caller)
		...
		timer.Simple(2, function() sound.Play("snd_jack_itemsget.wav", Pos, 75, 100) end)
		self:Remove()
	end
...
if CLIENT then
	...
	function ENT:Draw()
		...
		JMod.HoloGraphicDisplay(self, Vector(20, -2, 0), Angle(90, 90, 0), .05, 300, function() draw.SimpleText(Name, "JMod-Stencil", 0, 0, TxtCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP) end)
		JMod.HoloGraphicDisplay(self, Vector(-20, 2, 0), Angle(0, -90, -90), .05, 300, function() draw.SimpleText(Name, "JMod-Stencil", 0, 0, TxtCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP) end)
	end
