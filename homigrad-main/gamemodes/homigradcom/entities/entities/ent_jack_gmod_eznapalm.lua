-- "gamemodes\\homigradcom\\entities\\entities\\ent_jack_gmod_eznapalm.lua"
...
if SERVER then
	...
	function ENT:Detonate(tr)
		...
		if tr and tr.Hit then
			...
			timer.Create("gascreate"..self:EntIndex(),1,2,function()
				local Gas = ents.Create("ent_jack_gmod_ezfiregas")
				if IsValid(Gas) then
					Gas:SetPos(tr.HitPos + tr.HitNormal * 2)
					Gas:Spawn()
					Gas:Activate()
				end
			end)
