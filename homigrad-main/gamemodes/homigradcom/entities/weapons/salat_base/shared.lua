-- "gamemodes\\homigradcom\\entities\\weapons\\salat_base\\shared.lua"
...
ftlerped = 1 / 66
hook.Add("PlayerSwitchWeapon", "damksgmsddfh", function(ply)
...
function SWEP:PrimaryAttack()
	...
	if CLIENT and ply == LocalPlayer() then
		...
		local xmforce = self.Primary.Force/15
		local mulup = 6
		if self.TwoHands then
			xmforce = self.Primary.Force/45
			mulup = 3
		end
		
		AngleShoot = AngleShoot + Angle(xmforce*mulup, math.random(-xmforce,xmforce), 0)
...
function SWEP:IsSprinting()
	local owner = self:GetOwner()
	return hg.KeyDown(owner, IN_SPEED) or (self:GetNWFloat("DeployTime", 0) or 0) + 0.4 > CurTime()
end
...
function SWEP:Step()
	...
	if self:IsLocal() then
		if self.UpdateBoneScale then
			for _, boneName in ipairs(boneNames) do
				local boneIndex = ply:LookupBone(boneName)
				if boneIndex then
					ply:ManipulateBoneScale(boneIndex, Vector(1.25, 1.25, 1.25))
				end
			end
		end
	end
...
function SWEP:ApplyAnim(ply)
	...
	if self:IsLocal() then
		if not self:GetNWBool("IsSighted") then
			ply:ManipulateBoneAngles(hand_index, hand+(AngleShoot/1.5)+Angle(0,0,math.Clamp(GetConVar("grad_gunrotate"):GetInt(),-25,0)), false)
			ply:ManipulateBoneAngles(forearm_index, forearm+(AngleShoot/6), false)
		end
	end
end
...
if CLIENT then
	...
	function SWEP:Camera(ply, origin, angles, fov)
		...
		addfov = LerpFT(0.1, addfov, self:IsSighted() and -(self.addfov or 0) or 0)
