-- "gamemodes\\homigradcom\\entities\\weapons\\food_monster.lua"
...
if CLIENT then
	SWEP.PrintName = language.GetPhrase("hg.monster.name")
	...
	if SERVER then
		...
		if self.FoodMuch > 0 then
			if self.DelayFood < CurTime() then
				...
				sound.Play("snd_jack_hmcd_drink" .. math.random(1, 3) .. ".wav", self:GetPos(), 75, 100, 0.5)
			end
		end
		if self.FoodMuch <= 0 then
